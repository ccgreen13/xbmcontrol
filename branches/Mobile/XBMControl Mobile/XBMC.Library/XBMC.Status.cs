...
namespace XBMC
{
    public class XBMC_Status
    {
        ...
        private int connectedInterval = 1000;
        ...
        public void Refresh()
        {
            if (isConnected)
            {
                string currentMediaPlaying = parent.NowPlaying.Get("filename", true);

                if (mediaNowPlaying != currentMediaPlaying || mediaNowPlaying == null)
                {
                    mediaNowPlaying = currentMediaPlaying;
                    newMediaPlaying = true;
                }
                else
                    newMediaPlaying = false;

                string playStatus = parent.NowPlaying.Get("playstatus");

                isPlaying = (playStatus == "Playing") ? true : false;
                isPaused = (playStatus == "Paused") ? true : false;
                ...
                string aProgress = parent.NowPlaying.Get("percentage");
                if (aProgress == null || aProgress == "Error" || aProgress == "0" || Convert.ToInt32(aProgress) > 99)
                    progress = 1;
                else
                    progress = Convert.ToInt32(aProgress);
        ...
        public void SetVolumeStatus(int percentage)
        {
            volume = percentage;
        }
        ...
        public int GetVolume(bool refresh)
        {
            if (refresh)
            {
